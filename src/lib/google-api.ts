...
/* eslint-disable no-empty, no-async-promise-executor, no-case-declarations */

// Google API helper — authentication, Drive API, Picker (ported from Strata)
// Handles authentication, Drive API operations, and Picker integration

import { CLIENT_ID, API_KEY, SCOPES } from "./google-config";
import { DEBUG_SYNC } from "./google-sync-constants";
...
const STORAGE_KEY_TOKEN = "ideaforge_access_token";
const STORAGE_KEY_USER = "ideaforge_user_info";
const STORAGE_KEY_EXPIRY = "ideaforge_token_expiry";
...
// Get or create root folder "IdeaForge Data" in visible My Drive (not appDataFolder)
const getOrCreateRootFolder = async () => {
    ...
    rootFolderCreationLock = (async () => {
        try {
            ...
            const response = await gapi.client.drive.files.list({
                q: "name='IdeaForge Data' and mimeType='application/vnd.google-apps.folder' and 'root' in parents and trashed=false",
                fields: 'files(id, name)',
                pageSize: 1
            });
